...
import { HtmlTooltip } from '../controls';
import { useQuery } from 'react-query';

import { getRows, getFilters } from './GetData';
...
function HeadCell(props) {
    let { column, sorted, sort, small, sortIndex, sections } = props;
    let sortDir = sorted ? sorted.dir : undefined;
    const options = useMemo(() => column.options || {}, [column]);
    const columnRender = useMemo(
        () => options.customHeadRender ? options.customHeadRender() : column.label,
...
function Filter(props) {
    let { c, filter, multiFilter, currentFilter } = props;
    let start = '';
    let end = '';
    let min = '';
    let max = '';
                    ...
                    renderOption={(props, option, { selected }) => (
                        <Box display="flex" alignItems="center" justifyContent="space-between" {...props}>
                            ...
                            }
                            {
                                option == "Все" ?
                                    <div>Все</div> :
                                    <Box height={15} width={200} style={{ backgroundColor: option }}></Box>
                            }
                        </Box>
                    )}
                    onChange={(e, val) => {
                        if ((val && val === "Все") || (currentFilter && currentFilter.value.length === 1 && currentFilter.value.find(v => v === val))) {
                            props.changeFilter({ name: filter.column, value: !multiFilter ? "" : "all", label: filter.label, type: "color" });
                        } else {
                            val && props.changeFilter({ name: filter.column, value: val, label: filter.label, type: "color" });
                        }
                    }}
...
const SET_DETAIL_ROWS = 'SET_DETAIL_ROWS_ACTION';
...
const NerisTable = forwardRef((props, ref) => {
    ...
    const {
        showLoader: propShowLoader,
        stopLoader: propStopLoader,
        data,
        filtersWithData,
        search: propSearch,
        sort: propSort,
    ...
    const [currentSearch, setCurrentSearch] = useState(propSearch || "");
    ...
    const viewSectionTooltipText = useMemo(() => viewSectionTooltip || "Отображение секций", []);
    ...
    }, [propShowLoader, propStopLoader, reactContext]);
    ...
    const queryRows = useQuery(
        [data, 'list', queryOptions],
        getRows(data, queryOptions, disableToken),
        {
            enabled: typeof (data) === 'string',
            onError: (error) => {
                reactContext && reactContext.showMessage('Ошибка загрузки данных', 'error')
            },
            onSuccess: (data) => {
                setRows(data.rows);
                setTotal(data.total);
    ...
    useEffect(() => {
        setLoading(queryRows.isFetching);
    ...
    useQuery(
        [data, 'filters'],
        getFilters(data, disableToken),
        {
            enabled: typeof (data) === 'string' && !filtersWithData,
            onSuccess: (data) => {
    ...
    useEffect(() => {
        ...
        let newRows = disablePaging ? rows : rows.slice(table.rowsPerPage * table.page, table.rowsPerPage * (table.page + 1));

        if (detailRow === true) {
            let modifiedRows = [...rows];
            const childrenIds = []
            const parents = modifiedRows.filter((p) => p[childField].length > 0);

            parents.forEach(parent => {
                parent[childField].forEach(child =>
                    childrenIds.push(child[idField]));
            });

            const otherParents = modifiedRows.filter(item =>
                item[childField].length === 0 && !childrenIds.includes(item[idField]))
            newRows = [...parents, ...otherParents];
            if (newRows.length > 0) {
                newRows = newRows.flatMap((parent) =>
                    table.detailRows.find(dr => dr[idField] === parent[idField]) ?
                        [
                            parent,
                            ...parent.childIndustries.map((child) => {
                                child.isChild = true
                                return child
                            })
                        ] :
            ...
            if (filters.find(f => f.column === name && (type == null || type === f.type))) {
                filters = filters.map(f => {
                    if (f.column === name && (!f.value.includes(value) || value == null) && (type == null || f.type === type)) {
                        return {
                            column: name,
                            value: multiFilter && (type == null || type === "color") ? [...f.value, value] : [value],
                            type,
                            start: start || f.start,
                            end: end || f.end,
                            min: min || f.min,
                    ...
                    } else if (f.column === name && f.value.includes(value) && (type == null || type === "color")) {
                        return {
                            column: name,
        ...
        return {
            ...
            'date': <>
                {f.start && <Chip
                    ...
                    style={chipStyle}
                    label={"Позже " + format(new Date(f.start), 'dd.MM.yyyy')}
                    onDelete={() => { DeleteFilter(f, 'start'); }}
                />}
                {f.end && <Chip
                    key={"col-" + f.column + "-chip-end"}
                    style={chipStyle}
                    label={"Раньше " + format(new Date(f.end), 'dd.MM.yyyy')}
                    onDelete={() => { DeleteFilter(f, 'end'); }}
                />}</>,
            "color": f.value.map((fv, fvInd) =>
                <Chip
                    key={"col-" + f.column + "-chip-" + fvInd}
                    style={{ ...chipStyle, backgroundColor: fv }}
                    label="        "
        ...
        <Paper>
            {
                ...
                <Toolbar style={{ display: "flex", justifyContent: "space-between" }}>
                    ...
                    <Box display="flex">
                        ...
                        {
                            table.sections &&
                            <Tooltip title={viewSectionTooltipText}>
                                <IconButton ref={sectionsButtonElement} onClick={() => setSectionsOpen(true)}>
                                    <ViewColumn />
                                </IconButton>
                            </Tooltip>
                        }
