...
2. **sortBy: string** — если задано, то колонка начальной сортировки таблицы
3. **sortDir: "asc" || "desc"** — если задано, то направление начальной сортировки таблицы
4. **title: string** — если задано, то будет выведен заголовок страницы с иконкой поиска, в противном случае поиск будет доступене всегда
...
/>
```
3. **initialFilters ([Filter](#Filter))** — текцщие фильтры по таблице
4. **onFilterChanged (function(filters: arrayOf([Filter](#Filter))))** — если функция задана вместе с initialFilters, таблица для поиска значений будет использовать initialFilters для каждого обновления и возвращать результат изменения фильтрации значения в родительский компоненет
```js
<Table
...
/>
```
3. **sort ([Filter](#Filter))** — текущая сортировка таблицы
